...
| Table | Purpose |
|---|---|
| `users` | All user accounts (fund raiser, donor, donee, user admin, platform management) with role and status |
| `categories` | FRA categories managed by platform management |
| `fund_raising_activities` | Campaigns created by fund raisers, linked to a donee beneficiary |
| `donations` | Donations made by donors including anonymous flag and flagged reason |
| `payment_details` | Tokenized saved payment info per user |
| `favorites` | FRAs shortlisted/saved by donees |
| `ratings` | Donee ratings on donation experience |
| `campaign_updates` | Updates posted by fund raisers on their FRA |
| `thank_you_messages` | Thank-you messages from fund raisers to donors |
| `reported_campaigns` | FRAs reported by users, reviewed by admin |
| `user_violations` | Violation records for flagged users |
| `user_preferences` | Language, location, and category preferences per user |
| `platform_reports` | Daily/weekly/monthly activity reports generated by platform management |
