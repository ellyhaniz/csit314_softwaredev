...
import { searchFRAs } from '../../lib/api';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    searchFRAs({})
      .then((data) => {
        const userFRAs = Array.isArray(data)
          ? data.filter((f) => f.fund_raiser_id === user?.id)
          : [];
        setFras(userFRAs);
      })
      .catch(() => setFras([]))
  ...
  const filtered = fras.filter((f) => f.status?.toLowerCase() === filter.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {filtered.map((fra) => (
            <div key={fra.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 text-xs border border-gray-200 rounded hover:border-gray-400 transition-colors text-gray-600"
                  >
                    Edit
                  </button>
                </div>
