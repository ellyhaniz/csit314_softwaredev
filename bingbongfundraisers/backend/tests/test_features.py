...
def _make_db_mock(fetchone_returns):
    """Return a mock get_db context manager whose cursor.fetchone() yields
    successive values from *fetchone_returns* on each call."""
    cur = MagicMock()
    ...
    mock_get_db = MagicMock(return_value=conn)
    return mock_get_db, cur
...
class TestPR01_RegisterNewUser:
    ...
    def test_NF__valid_details_unique_email__account_created_and_data_returned(
        ...
        mock_get_db, cur = _make_db_mock([
            None,  # first fetchone: email-exists check returns None (email free)
    ...
    def test_AF4A__email_already_registered__raises_409(self, mock_get_db):
        ...
        mock_get_db, cur = _make_db_mock([
            {"id": 5},  # email-exists check returns an existing row
        ])
